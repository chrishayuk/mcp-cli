...
class TestChatHelpCommand:
    ...
    async def test_help_no_arguments(self, mock_output, mock_context):
        ...
        assert any("Available Commands" in s or "/help <command>" in s for s in call_strings)
    ...
    async def test_help_specific_command(self, mock_output, mock_context):
        ...
        panel_calls = [call for call in mock_output.print.call_args_list 
                       if "Panel" in str(call)]
    ...
    async def test_help_tools_group(self, mock_output, mock_context):
        ...
        assert "Panel" in str(panel_call) or "Tool Commands" in str(mock_output.print.call_args_list)
    ...
    @patch("mcp_cli.chat.commands.help.output")
    async def test_help_panel_creation(self, mock_output, mock_markdown, mock_panel, mock_context):
    ...
    @patch("mcp_cli.chat.commands.help.output")
    async def test_help_table_creation(self, mock_output, mock_table_class, mock_context):
