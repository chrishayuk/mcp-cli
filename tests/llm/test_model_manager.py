...
class TestProviderManagement:
    ...
    def test_set_active_provider(self, mock_manager):
        ...
        with patch.object(
            mock_manager, "get_available_models", return_value=["model1"]
        ):
...
class TestModelManagement:
    ...
    def test_get_available_models_ollama(self, mock_manager_with_models):
        ...
        with patch("chuk_llm.llm.client.list_available_providers") as mock_list:
            mock_list.return_value = {
                "ollama": {"models": ["llama3.3", "mistral", "gpt-oss", "qwen3"]}
            }
    ...
    def test_get_available_models_openai(self, mock_manager_with_models):
        ...
        with patch("chuk_llm.llm.client.list_available_providers") as mock_list:
            mock_list.return_value = {
                "openai": {"models": ["gpt-4o", "gpt-5-mini", "gpt-5", "gpt-3.5-turbo"]}
            }
            ...
            if "gpt-5" in models:
                ...
                gpt4_index = (
                    models.index("gpt-4o") if "gpt-4o" in models else len(models)
                )
...
class TestDiscoveryAndRefresh:
    ...
    @patch("chuk_llm.configuration.get_config")
    def test_refresh_models_ollama(
        self, mock_config, mock_refresh, mock_ollama_discovery
    ):
    ...
    @patch("chuk_llm.api.providers.refresh_provider_functions")
    @patch("chuk_llm.configuration.get_config")
    def test_refresh_models_other_provider(
        self, mock_config, mock_refresh, mock_ollama_discovery
    ):
...
class TestClientManagement:
    ...
    def test_get_client_creates_and_caches(
        self, mock_get_client, mock_manager_for_clients
    ):
        """Test client creation and caching."""
    ...
    @patch("chuk_llm.llm.client.get_client")
    def test_get_client_with_explicit_params(
        self, mock_get_client, mock_manager_for_clients
    ):
