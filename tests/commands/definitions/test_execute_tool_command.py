...
from mcp_cli.commands.base import CommandMode, CommandResult
...
def mock_tool_manager(mock_tool):
    ...
    another_tool.inputSchema = None  # Explicitly set to None to avoid MagicMock auto-creation
...
class TestExecuteToolCommand:
    ...
    @pytest.mark.asyncio
    async def test_execute_invalid_json_params(self, execute_command, mock_tool_manager):
    ...
    @pytest.mark.asyncio
    async def test_execute_plain_string_params(self, execute_command, mock_tool_manager):
        ...
        with patch("mcp_cli.commands.definitions.execute_tool.output") as mock_output:
            ...
            assert result.success is False
            mock_output.error.assert_called_with("❌ Invalid format: Parameters must be in JSON format")
    ...
    async def test_execute_tool_failure(self, execute_command, mock_tool_manager):
        ...
        mock_tool_manager.execute_tool = AsyncMock(return_value=ToolCallResult(
            tool_name="test_tool",
            success=False,
    ...
    async def test_execute_tool_exception(self, execute_command, mock_tool_manager):
        ...
        mock_tool_manager.execute_tool = AsyncMock(side_effect=Exception("Unexpected error"))
    ...
    @pytest.mark.asyncio
    async def test_execute_get_all_tools_exception(self, execute_command, mock_tool_manager):
        """Test handling exceptions when getting tools."""
        mock_tool_manager.get_all_tools = AsyncMock(side_effect=Exception("Connection error"))
    ...
    @pytest.mark.asyncio
    async def test_execute_missing_required_param(self, execute_command, mock_tool_manager):
    ...
    @pytest.mark.asyncio
    async def test_execute_parse_simple_params(self, execute_command, mock_tool_manager):
