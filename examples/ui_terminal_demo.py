...
class TerminalDemo:
    ...
    async def demo_clear_screen(self):
        ...
        ui.print("  • Windows: cls")
        ui.print("  • Unix/Linux/macOS: clear")
        ui.print()
        ui.print(
            f"Your platform ({sys.platform}) used: {'cls' if sys.platform == 'win32' else 'clear'}"
        )
    ...
    async def demo_terminal_reset(self):
        ...
        if sys.platform == "win32":
            ...
            ui.print(
                "On Unix-like systems, this would run 'stty sane' to restore terminal settings."
            )
        else:
    ...
    async def demo_size_detection(self):
        ...
        if cols < 80:
            ...
            ui.success(
                f"  Wide terminal ({cols} cols) - Excellent for side-by-side views"
            )
    ...
    async def demo_hyperlinks(self):
        ...
        ui.print(
            "  • Project repository: [link=https://github.com/anthropics/mcp-cli]https://github.com/anthropics/mcp-cli[/link]"
        )
        ui.print()

        # Hyperlink with custom text
        ui.print(
            "  • Documentation: [link=https://docs.anthropic.com]Anthropic Docs[/link]"
        )
        ui.print()

        # Email link
        ui.print("  • Email: [link=mailto:support@example.com]Contact Support[/link]")
        ui.print()

        # File link (local)
        ui.print("  • Local folder: [link=file:///Users]Open Users Folder[/link]")
        ui.print()

        ui.print(
            "[dim]Try clicking the links above (if your terminal supports it)![/dim]"
        )
        ui.print()
        ...
        ui.print(
            "The TerminalManager.hyperlink() function creates raw OSC 8 sequences."
        )
        ui.print(
            "When using Rich, use [dim][link=url]text[/link][/dim] markup instead."
        )
    ...
    async def demo_asyncio_tasks(self):
        ...
        ui.print()
        ui.print(
            "[dim]Note: Terminal cleanup automatically handles remaining tasks on exit.[/dim]"
        )
    ...
    async def demo_stress_test(self):
        ...
        try:
            while True:
                ...
                if operations % 10 == 0:
                    ...
                    ui.print(
                        f"Rate: {operations / (time.time() - start_time):.1f} ops/sec"
                    )
        ...
        ui.print()
        ui.success("Stress test complete!")
...
async def main():
    ...
    try:
        ...
        if not sys.stdout.isatty():
            ui.warning(
                "Warning: Not running in a TTY. Some features may not work correctly."
            )
