...
class OutputDemo:
    ...
    async def demo_message_levels(self):
        ...
        self.ui.print(
            "MCP CLI provides different message levels for various scenarios:"
        )
        self.ui.print()
        ...
        self.ui.print()
        self.ui.print(
            "[dim]Note: Debug messages are hidden unless verbose mode is enabled.[/dim]"
        )
    ...
    async def demo_panels(self):
        ...
        # Simple panel
        self.ui.panel(
            "This is a simple panel with important information.", title="Notice"
        )
        self.ui.print()

        # Styled panel
        self.ui.panel(
            "This panel has custom styling applied.", title="Styled Panel", style="cyan"
        )
        self.ui.print()
...
"""
    ...
    async def demo_progress(self):
        ...
        for spinner_type in spinners:
            with self.ui.loading(
                f"Using {spinner_type} spinner...", spinner=spinner_type
            ):
...
""",
    ...
    async def demo_advanced_formatting(self):
        ...
        self.ui.print(
            "[dim]Uses formatters.format_tree for consistent tree rendering[/dim]"
        )
        self.ui.print()
        ...
        self.ui.list_items(
            ["Install dependencies", "Configure settings", "Run application"],
            style="number",
        )
        self.ui.print()
        ...
        self.ui.print(
            "[dim]Uses code.display_code for language-aware highlighting[/dim]"
        )
        self.ui.print()
        ...
        if self.ui.confirm(f"Process {num_files} files?", default=True):
            ...
            for i in range(int(num_files)):
                ...
                if i == 2:  # Simulate one warning
                    table.add_row(
                        file_name, file_size, "⚠ Warning", f"{process_time:.2f}s"
                    )
                    warning(f"Non-critical issue in {file_name}")
                else:
                    table.add_row(
                        file_name, file_size, "✓ Complete", f"{process_time:.2f}s"
                    )
