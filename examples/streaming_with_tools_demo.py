...
async def demo_tool_streaming():
        ...
        async def create_completion(self, messages, tools=None, stream=True, **kwargs):
            ...
            if "echo" in user_msg.lower() or "database" in user_msg.lower():
                ...
                thinking_chunks = [
                    ...
                    {
                        "response": "I'll use the echo tool to process",
                        "tool_calls": None,
                    },
                    {
                        "response": "I'll use the echo tool to process your",
                        "tool_calls": None,
                    },
                    {
                ...
                chunks = [
                    ...
                    {
                        "response": "This is a normal response without",
                        "tool_calls": None,
                    },
                    {
                        "response": "This is a normal response without tools.",
                        "tool_calls": None,
                    },
                ]
    ...
    result = await handler.stream_response(client=client, messages=messages, tools=[])

    output.info(
        f"Result: {result['chunks_received']} chunks, {len(result.get('tool_calls', []))} tool calls"
    )
    ...
    result = await handler.stream_response(
        client=client, messages=messages, tools=tools
    )
    ...
    if result.get("tool_calls"):
        ...
        for i, tc in enumerate(result["tool_calls"]):
            ...
            output.print(
                f"  Tool {i + 1}: {func.get('name')} with args: {func.get('arguments')}"
            )
