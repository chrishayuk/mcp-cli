...
class CommandSystemVisualDemo:
    ...
    async def run_demo(self):
        ...
        if not self.api_key:
            ...
            console.print("[yellow]Please set OPENAI_API_KEY or add to .env file[/yellow]")
            return
        ...
        self.pause("Notice how 'models ▸', 'providers ▸', and 'tools ▸' have indicators...")
        
        self.execute_and_display(
            "/help models",
            "Now let's get detailed help for the models command"
        )
        
        # ====== 2. MODEL MANAGEMENT ======
        self.demo_section(
            "2. Model Management",
            "Direct model switching without 'set' subcommand"
        )
        
        self.execute_and_display(
            "/models",
            "First, let's see our current model"
        )
        
        self.execute_and_display(
            "/model gpt-4o",
            "Switch directly to gpt-4o (no 'set' needed!)"
        )
        ...
        self.execute_and_display(
            "/model list",
            "List all available models using the list subcommand"
        )
        
        # ====== 3. PROVIDER MANAGEMENT ======
        self.demo_section(
            "3. Provider Management",
            "Direct provider switching and status"
        )
        ...
        self.execute_and_display(
            "/provider list",
            "See all available providers with their status"
        )
        
        self.execute_and_display(
            "/provider ollama",
            "Switch to Ollama provider directly (no 'set' needed!)"
        )
        
        self.execute_and_display(
            "/models",
            "Models change when we switch providers"
        )
        ...
        self.demo_section(
            "4. Server Management",
            "MCP server information and connectivity"
        )
        ...
        self.execute_and_display(
            "/prompts",
            "Check for any available prompts"
        )
        ...
        # ====== 6. UI COMMANDS ======
        self.demo_section(
            "6. UI & Utility Commands",
            "Theme switching and display options"
        )
        ...
        self.execute_and_display(
            "/verbose",
            "Toggle verbose mode for more detailed output"
        )
        ...
        self.execute_and_display(
            "/",
            "Just a slash shows the command menu"
        )
