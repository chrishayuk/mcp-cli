...
from chuk_llm import conversation, ask, tools_from_functions, Tools, tool
...
async def demo_conversation_memory():
    ...
    async with conversation("ollama", "gpt-oss") as conv:
        ...
        print(f"🤖: {response if isinstance(response, str) else response.get('response', '')}")
    ...
    if isinstance(result, dict) and "tool_calls" in result:
        ...
        for tc in result["tool_calls"]:
            ...
            if func.get("name") == "get_time":
                ...
                print(f"   ✅ Tool called: get_time()")
                print(f"   → Result: {time_result['time']}")
    else:
        print(f"🤖: {result if isinstance(result, str) else result.get('response', '')}")
