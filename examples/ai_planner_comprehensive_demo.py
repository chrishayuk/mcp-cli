...
class SmartPlanner:
    ...
    def create_plan(self) -> List[Task]:
        ...
        while len(sorted_tasks) < len(self.tasks):
            ...
            if available:
                ...
                available.sort(key=lambda t: (t.priority, -len([x for x in self.tasks if t.id in x.dependencies])))
    ...
    def calculate_savings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_percent": int((1 - len(required_tools) / len(all_tools)) * 100),
            "cost_used": used_cost,
...
async def demo_planning_with_todos():
    ...
    output.print("I'll create a comprehensive plan to refactor your authentication system.")
    output.print("")
    ...
    tasks = [
        ...
        Task("2", "Install JWT dependencies", priority=1, tool="run_shell", duration=0.5),
        Task("3", "Create JWT utilities", priority=2, dependencies=["1", "2"], tool="write_file", duration=1.5),
        Task("4", "Update login endpoint", priority=3, dependencies=["3"], tool="write_file", duration=1.0),
        Task("5", "Update middleware", priority=3, dependencies=["3"], tool="write_file", duration=1.0),
        Task("6", "Write tests", priority=4, dependencies=["4", "5"], tool="write_file", duration=1.0),
        Task("7", "Run tests", priority=5, dependencies=["6"], tool="run_tests", duration=2.0),
        Task("8", "Update documentation", priority=6, dependencies=["7"], tool="write_file", duration=0.5),
    ]
    ...
    for task in planner.create_plan():
        status_icon = "⏳"
        deps_str = f"After: {', '.join(f'#{d}' for d in task.dependencies)}" if task.dependencies else "Ready"
    ...
    output.print("")
    output.hint(f"📊 Optimization: Using only {savings['tools_used']}/{savings['tools_available']} tools (saved {savings['optimization_percent']}%)")
    output.print("")
    ...
    for task in planner.create_plan():
        ...
        if task.tool:
            ...
            output.print(f"   [dim]Using reasoning...[/dim]")
...
async def demo_adaptive_planning():
    ...
    initial_tasks = [
        Task("1", "Create project structure", tool="create_directory", status="completed"),
    ...
    new_tasks = [
        ...
        Task("3b", "Create migration config", tool="write_file", priority=2, dependencies=["3a"]),
        Task("3c", "Generate initial migration", tool="run_shell", priority=3, dependencies=["3b"]),
    ]
...
async def demo_planning_without_tools():
    ...
    tasks = [
        ...
        Task("8", "Document architecture decisions", priority=5, dependencies=["6", "7"]),
    ]
    ...
    for i, group in enumerate(groups, 1):
        ...
        for task in group:
            deps = f" [dim](requires: {', '.join(task.dependencies)})[/dim]" if task.dependencies else ""
...
async def demo_parallel_execution():
    ...
    services = ["auth-api", "payment-service", "user-service", "notification-api", "analytics-engine"]
    
    # Setup phase
    planner.add_task(Task("setup", "Initialize security scanners", tool="security_scan", duration=1.0))
    
    # Parallel scanning (all depend only on setup)
    for i, service in enumerate(services, 1):
        planner.add_task(Task(f"scan_{i}", f"Scan {service}", 
                             tool="security_scan", 
                             dependencies=["setup"],
                             duration=2.0))
    ...
    planner.add_task(Task("aggregate", "Aggregate findings", 
                         dependencies=scan_ids, duration=1.0))
    planner.add_task(Task("report", "Generate report", 
                         tool="write_file",
                         dependencies=["aggregate"], duration=0.5))
    ...
    for i, group in enumerate(groups, 1):
        if len(group) > 1:
            output.print(f"  Phase {i}: [green]PARALLEL[/green] - {len(group)} tasks simultaneously")
        else:
    ...
    for i, group in enumerate(groups, 1):
        if len(group) > 1:
            output.print(f"[yellow]Phase {i}: Running {len(group)} scans in parallel[/yellow]")
            for task in group:
                output.print(f"  🔍 {task.description.replace('Scan ', '')}: scanning...")
            await asyncio.sleep(0.5)
            for task in group:
                output.success(f"  ✓ {task.description.replace('Scan ', '')}: completed")
        else:
            task = group[0]
    ...
    output.success(f"🚀 Analysis completed {savings['speedup']:.1f}x faster with parallel execution!")
...
🎯 Smarter resource usage
    """.strip(), title="AI Planner", style="cyan")
...
Ready for production integration!
    """.strip(), title="Success", style="green")
