...
async def demo_tool_calling():
    ...
    output.print("I'll help you list Python files and analyze their complexity. Let me break this down into steps:")
    output.print("")
    
    # Show tool planning
    output.hint("Planning tool execution:")
    tools_to_execute = [
        {"name": "list_files", "args": {"pattern": "**/*.py"}, "description": "List all Python files"},
        {"name": "read_file", "args": {"path": "src/main.py"}, "description": "Read main.py"},
        {"name": "analyze_complexity", "args": {"files": ["main.py", "utils.py"]}, "description": "Analyze code complexity"},
    ]
...
async def demo_streaming_response():
    ...
    # User request
    output.print("[bold cyan]User[/bold cyan]")
    output.print("Explain how to implement async file operations in Python with an example")
    output.print("")
    
    # Assistant response
    output.print("[bold green]Assistant[/bold green]")
    output.print("I'll explain async file operations in Python and provide a practical example.")
    output.print("")
...
if __name__ == '__main__':
    ...
    output.print("")
    output.print("This approach allows you to handle file operations efficiently without blocking your application's event loop.")
...
async def demo_code_modifications():
    ...
    # Show original code
    output.print("[bold green]Assistant[/bold green]")
    output.print("I'll add comprehensive error handling and logging. Here's the original function:")
    output.print("")
...
async def demo_adaptive_planning():
    ...
    for todo in todos:
        if todo["task"] in [t["task"] for t in new_tasks]:
            output.print(f"  {todo['icon']} [green]+[/green] {todo['task']} [dim](new)[/dim]")
        else:
            output.print(f"  {todo['icon']} {todo['task']}")
...
async def demo_tool_cancellation():
    ...
    for i, tool in enumerate(tools[:3]):
        ...
        output.print(f"  [dim]Processing...[/dim]")
        await asyncio.sleep(0.5)
    ...
    output.print("")
    output.print("[dim]You can resume the analysis with 'continue' or start fresh with 'restart'[/dim]")
...
async def demo_successful_tool_execution():
    ...
    # Assistant response
    output.print("[bold green]Assistant[/bold green]")
    output.print("I'll help you create a feature branch, make changes, and create a pull request.")
    output.print("")
    ...
    for tool in tools_sequence:
        ...
        if tool['result_type'] == 'info':
            ...
            output.success(f"  ✓ File modified successfully")
            diff = tool['result']
            output.print(f"    [green]+{diff['added']}[/green] [red]-{diff['removed']}[/red] [yellow]~{diff['modified']}[/yellow] lines changed")
            
        elif tool['result_type'] == 'test':
            test = tool['result']
            output.success(f"  ✓ All tests passed!")
            output.print(f"    [green]✓ {test['passed']} passed[/green] | [dim]{test['skipped']} skipped[/dim] | Time: {test['time']}")
            
        elif tool['result_type'] == 'url':
            output.success(f"  ✓ Pull request created")
...
async def demo_error_recovery():
    ...
    # Assistant response
    output.print("[bold green]Assistant[/bold green]")
    output.print("I'll help you deploy the application to production. Let me run through the deployment checklist.")
    output.print("")
    ...
    checks = [
        ...
        {"name": "test_suite", "status": "fail", "message": "2 tests failing", "details": [
            "FAILED: test_async_handler.py::test_concurrent_writes",
            "FAILED: test_async_handler.py::test_error_handling"
        ]},
        {"name": "security_scan", "status": "skip", "message": "Skipped due to test failures"},
...
async def demo_planner_and_execution():
    ...
    # User request
    output.print("[bold cyan]User[/bold cyan]")
    output.print("Refactor the authentication system to use JWT tokens instead of sessions")
    output.print("")
    
    # Assistant planning phase
    output.print("[bold green]Assistant[/bold green]")
    output.print("I'll help you refactor the authentication system to use JWT tokens. Let me create a comprehensive plan for this task.")
    output.print("")
    
    # Define the plan as todo items
    todos = [
        {"id": 1, "task": "Analyze current session-based auth implementation", "status": "pending", "deps": []},
        {"id": 2, "task": "Install JWT dependencies (pyjwt, python-jose)", "status": "pending", "deps": []},
        {"id": 3, "task": "Create JWT token generation utilities", "status": "pending", "deps": [2]},
        {"id": 4, "task": "Create JWT token validation middleware", "status": "pending", "deps": [3]},
        {"id": 5, "task": "Update user login endpoint to issue JWT tokens", "status": "pending", "deps": [3]},
        {"id": 6, "task": "Update protected endpoints to use JWT validation", "status": "pending", "deps": [4]},
        {"id": 7, "task": "Implement token refresh mechanism", "status": "pending", "deps": [3, 4]},
        {"id": 8, "task": "Update frontend to store and send JWT tokens", "status": "pending", "deps": [5]},
        {"id": 9, "task": "Write tests for JWT authentication", "status": "pending", "deps": [3, 4, 5, 6]},
        {"id": 10, "task": "Update API documentation", "status": "pending", "deps": [5, 6, 7]},
        {"id": 11, "task": "Run security audit on new implementation", "status": "pending", "deps": [9]},
        {"id": 12, "task": "Deploy to staging and test", "status": "pending", "deps": [11]},
    ]
    ...
    for todo in todos:
        ...
        deps_str = f"Depends on: {', '.join(f'#{d}' for d in todo['deps'])}" if todo['deps'] else "No dependencies"
        table_data.append({
            "#": str(todo["id"]),
            "Status": status_icon,
    ...
    while len(completed) < len(todos):
        ...
        if available_tasks and in_progress is None:
            ...
            output.print(f"   [dim]Starting execution...[/dim]")
            ...
            if task["id"] == 9:  # Tests might find issues
                output.warning(f"   ⚠️  Found 2 failing tests, fixing...")
                await asyncio.sleep(0.3)
                output.success(f"   ✓ Tests fixed and passing")
            elif task["id"] == 11:  # Security audit
                output.print(f"   [dim]Running security scan...[/dim]")
                await asyncio.sleep(0.3)
                output.success(f"   ✓ No vulnerabilities found")
            else:
                # Normal completion
                output.success(f"   ✓ Completed successfully")
            ...
            total = len(todos)
            progress_bar = "█" * (progress * 20 // total) + "░" * ((total - progress) * 20 // total)
            output.print(f"")
            output.print(f"[dim]Progress: [{progress_bar}] {progress}/{total} tasks completed[/dim]")
            output.print("")
...
async def demo_parallel_tool_execution():
    ...
    services = [
        ...
        {"name": "payment-service", "port": 3003, "status": None, "response_time": None},
        {"name": "notification-service", "port": 3004, "status": None, "response_time": None},
        {"name": "analytics-service", "port": 3005, "status": None, "response_time": None},
    ]
    ...
    while len(completed) < len(services):
        ...
        if remaining:
            ...
            if service['status'] == 'healthy':
                output.success(f"  ✓ {service['name']:<20} [green]Healthy[/green] ({service['response_time']}ms)")
            else:
                output.warning(f"  ⚠ {service['name']:<20} [yellow]Degraded[/yellow] ({service['response_time']}ms)")
...
• Error recovery
• Parallel execution
    """.strip(), title="MCP-CLI Demo", style="cyan")
