...
def list_tables() -> dict:
    ...
    return {"tables": ["users", "posts", "comments", "tags"], "count": 4}


def create_table(table_name: str, columns: List[Dict[str, str]]) -> dict:
    """Create a new table."""
    print(f"   🔨 Creating table: {table_name}")
    return {"success": True, "table": table_name, "columns": len(columns)}
    ...
    if isinstance(result, dict) and "tool_calls" in result:
        ...
        for tc in result["tool_calls"]:
            ...
            if name == "list_tables":
                ...
                print(
                    f"   ✅ Found {table_result['count']} tables: {', '.join(table_result['tables'])}"
                )
    ...
    if isinstance(result, dict) and "tool_calls" in result:
        ...
        for tc in result["tool_calls"]:
            ...
            if name == "create_table":
                ...
                print(
                    f"   ✅ Table '{create_result['table']}' created with {create_result['columns']} columns"
                )
...
async def demo_conversation_flow():
    ...
    async with conversation("ollama", "gpt-oss") as conv:
        ...
        print(
            f"🤖: {response if isinstance(response, str) else response.get('response', '')[:100]}..."
        )
        ...
        print("🤖: The 'users' table contains user data")
...
async def demo_streaming_with_tools():
    ...
    async with conversation("ollama", "gpt-oss") as conv:
        ...
        async for chunk in conv.stream(
            "Explain what each table is for: users, posts, comments, tags"
        ):
