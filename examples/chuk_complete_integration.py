...
# Chuk libraries
from chuk_llm import conversation, tools_from_functions
...
/exit     - Exit the chat
    ...
    async def change_theme(self):
        ...
        selected = prompts.select_from_list(
            "Select a theme:", themes, current_value=theme.get_current_theme()
        )
    ...
    async def show_stats(self):
        ...
        if self.current_conversation:
            ...
            stats = {
                ...
                "Has Session": conv.has_session
                if hasattr(conv, "has_session")
                else False,
    ...
    async def handle_tool_response(self, conv, user_input: str):
        ...
        if isinstance(result, dict) and "tool_calls" in result:
            ...
            output.print_label("Assistant", style="green")
            response_text = (
                result if isinstance(result, str) else result.get("response", "")
            )
    ...
    def execute_tool(self, tool_name: str, args: dict) -> dict:
        ...
        tool_map = {
            ...
            "read_file": lambda: {
                "content": f"Content of {args.get('filename', 'unknown')}",
                "size": 1024,
            },
            "run_command": lambda: {
                "output": f"Executed: {args.get('command', 'unknown')}",
                "status": "success",
            },
        }
