...
async def demo_eviction_under_pressure() -> None:
    ...
    print(f"  Budget: {config.max_l0_tokens} tokens "
    ...
    messages = [
        ("user", "My name is Chris and I live in Leavenheath, a village in Suffolk, England. "
                 "I'd like to learn about various topics today including history, weather, and jokes."),
        ("ai", "Nice to meet you, Chris! Leavenheath is a lovely village in the Babergh district of Suffolk. "
               "I'd be happy to help you explore any topics you're interested in today. What shall we start with?"),
        ("user", "Tell me about the history of the New York Football Giants, especially their founding and early years "
                 "in the NFL. I'm particularly interested in the pre-Super Bowl era championships they won."),
        ("ai", "The New York Giants were founded in 1925 by Tim Mara, who purchased the franchise for just $500. "
        ...
        ("user", "What's the weather like where I am right now? I'm curious about the temperature, wind conditions, "
                 "and whether I should expect rain tonight or tomorrow morning in my area."),
        ("ai", "In Leavenheath right now it's 8.5 degrees Celsius with partly cloudy skies. "
        ...
        ("user", "Tell me a really good joke about cheese. Something that would make people at a dinner party laugh. "
                 "I want to impress my friends with my comedy skills this weekend."),
        ("ai", "Here are three cheese jokes for your dinner party: What type of cheese is made backwards? Edam! "
        ...
        ("user", "What's my name again? I want to make sure you remember our conversation from the beginning. "
                 "Also, where did I say I live? Just checking your memory."),
        ("ai", "Your name is Chris, and you told me at the start of our conversation that you live in Leavenheath, "
    ...
    evictions_before = mm.metrics.evictions_total
    ...
    assert evictions_after > 0, "Must have evictions with 10 messages in 500-token budget!"
    assert mm.working_set.tokens_used <= config.max_l0_tokens, "L0 must not exceed budget!"
...
def demo_context_building() -> None:
    ...
    for i, content in enumerate([
        ...
        mm._page_table.update_location(page.page_id, tier=mm._page_table.entries[page.page_id].tier)
...
async def demo_event_filtering() -> None:
    ...
    events = []
    for i in range(10):
        events.append(HistoryMessage(
            role=MessageRole.USER,
            content=f"User message {i}: " + "This is a detailed question about a complex topic. " * 4
        ))
        events.append(HistoryMessage(
            role=MessageRole.ASSISTANT,
            content=f"Assistant reply {i}: " + "Here is a comprehensive answer with supporting details. " * 4
        ))
    ...
    assert "9" in last_filtered, f"Last message must be from turn 9, got: {last_filtered}"
...
async def demo_full_integration() -> None:
    ...
    assert ws.budget.total_limit == 500, f"Budget must be 500, got {ws.budget.total_limit}"
    ...
    assert metrics.evictions_total > 0, "Must have evictions with 12 messages in 500-token budget!"
    assert ws_stats.tokens_used <= 500, f"L0 tokens ({ws_stats.tokens_used}) must not exceed budget (500)!"

    # History should be filtered (not all 12 messages)
    # System prompt + filtered events
    assert len(history) < 14, f"History ({len(history)}) should be filtered, not all 14 (1 sys + 12 msgs + 1 notice)"

    # First message should be system/developer message with VM:CONTEXT
    first = history[0]
    assert "<VM:CONTEXT>" in (first.content or ""), "First message must be VM developer_message"
