...
async def test_streaming_call(client, messages, tools):
    ...
    print(
        f"  ✅ Streaming complete: {len(all_chunks)} chunks, {len(tool_calls)} tool calls"
    )
...
async def test_even_simpler():
    ...
    try:
        ...
        # Ultra explicit
        messages = [{"role": "user", "content": "Call say_hello with name 'Alice'"}]
        ...
        if stream_result and regular_result:
            stream_name = json.loads(stream_result[0]["function"]["arguments"]).get(
                "name", ""
            )
            regular_name = json.loads(regular_result[0]["function"]["arguments"]).get(
                "name", ""
            )
