...
async def diagnose_tool_registry(servers: List[str], config_file: str):
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            "  ⚠️  Gateway URL returns 404 - might be normal for MCP endpoint"
                        )
                    else:
                    ...
                    print("  ❌ Gateway timeout after 10s")
    ...
    if hasattr(tool_manager, "stream_manager") and tool_manager.stream_manager:
        ...
        if hasattr(stream_manager, "_transports") or hasattr(
            stream_manager, "transport"
        ):
            ...
            if hasattr(stream_manager, "transport"):
                ...
                transport = (
                    list(stream_manager._transports.values())[0]
                    if stream_manager._transports
                    else None
                )
            ...
            if transport:
                ...
                print(
                    f"  Transport connected: {getattr(transport, 'is_connected', lambda: 'unknown')()}"
                )
                ...
                if hasattr(transport, "_initialized"):
                    print(
                        f"  Initialized: {getattr(transport, '_initialized', 'unknown')}"
                    )
    ...
    if tool_manager._registry:
        try:
            registry_items = await asyncio.wait_for(
                tool_manager._registry.list_tools(), timeout=30.0
            )
            ...
            if len(registry_items) > 0:
                ...
                if len(registry_items) > 3:
                    for i, (namespace, name) in enumerate(
                        registry_items[-2:], len(registry_items) - 1
                    ):
                        print(f"  {i}. namespace='{namespace}', name='{name}'")
    ...
    test_names = [
        "mcp-grid-dev-echo",
        "mcp-grid-dev-duckduckgo-search",
        "mcp-grid-dev-google-search",
    ]

    for test_name in test_names:
        try:
            namespace, resolved_name = await tool_manager._find_tool_in_registry(
                test_name
            )
            if namespace:
    ...
    for i, test_tool in enumerate(test_tools):
        ...
        if i > 0:
            ...
            tool_manager = ToolManager(
                config_file=config_file, servers=servers, tool_timeout=60.0
            )
            ...
            if hasattr(tool_manager, "stream_manager") and tool_manager.stream_manager:
                try:
                    ...
                    if transport and hasattr(transport, "pending_requests"):
                        ...
                        if pending_count > 0:
                            print(
                                f"  ⚠️  {pending_count} pending requests after tool call"
                            )
    ...
    try:
        ...
        for server in server_info:
            print(
                f"  {server.id}: {server.name} ({server.status}) - {server.tool_count} tools"
            )
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--config", help="Config file path (auto-detected if not specified)"
    )
