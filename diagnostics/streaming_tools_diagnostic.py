...
class ActualStreamingBugReproducer:
    ...
    async def _make_real_streaming_call(self):
        ...
        try:
            ...
            self.log_info(
                f"✅ Streaming call completed: {completion.get('streaming', False)}"
            )
            ...
            if tool_calls:
                self.log_info(
                    f"🔧 Processing {len(tool_calls)} tool calls from streaming"
                )
    ...
    async def _process_streaming_tool_calls(self, tool_calls: List[Dict[str, Any]]):
        ...
        try:
            ...
            self.log_info(
                "🚨 Calling tool_processor.process_tool_calls() - ERROR EXPECTED"
            )
