...
class ConversationFlowDiagnostic:
    ...
    async def diagnose_conversation_flow(
        ...
        self.log_info(
            "Reproducing: User types 'select top 10 products from the database'"
        )
    ...
    async def _simulate_user_input(self, user_input: str):
        ...
        self.log_debug(
            f"Conversation history length: {len(self.chat_context.conversation_history)}"
        )
    ...
    async def _load_tools_like_conversation(self):
        ...
        try:
            ...
            if hasattr(self.chat_context.tool_manager, "get_adapted_tools_for_llm"):
                ...
                tools_and_mapping = (
                    await self.chat_context.tool_manager.get_adapted_tools_for_llm(
                        provider
                    )
                )
                ...
                self.log_debug(
                    f"Loaded {len(self.chat_context.openai_tools)} adapted tools for {provider}"
                )
                self.log_debug(
                    f"Tool name mapping: {self.chat_context.tool_name_mapping}"
                )
    ...
    def _create_mock_streaming_response(self) -> Dict[str, Any]:
        ...
        self.log_debug(
            f"Mock response created with {len(mock_response['tool_calls'])} tool calls"
        )
    ...
    async def _test_tool_call_extraction_and_execution(self):
        ...
        tool_calls = [
            {
                "id": "call_list_tables_123",
                "type": "function",
                "function": {"name": "stdio_list_tables", "arguments": "{}"},
            },
            {
                "id": "call_read_query_456",
                "type": "function",
        ...
        try:
            ...
            self.log_error(
                "This is likely the source of the 'Missing required parameter' error!"
            )
            import traceback
