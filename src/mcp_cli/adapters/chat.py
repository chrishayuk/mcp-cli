...
class ChatCommandAdapter:
    ...
    @staticmethod
    async def handle_command(command_text: str, context: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        if isinstance(command, CommandGroup) and args:
            ...
            if subcommand_name in command.subcommands:
                ...
                if len(args) > 1:
                    kwargs.update(ChatCommandAdapter._parse_arguments(command.subcommands[subcommand_name], args[1:]))
            else:
    ...
    def _parse_arguments(command: Any, args: List[str]) -> Dict[str, Any]:
        ...
        while i < len(args):
            ...
            if arg.startswith("--"):
                ...
                param = next(
                    (p for p in command.parameters if p.name == option_name),
                    None
                )
    ...
    def get_completions(partial_text: str) -> List[str]:
        ...
        if len(parts) == 0 or (len(parts) == 1 and not partial.endswith(" ")):
            ...
            for name in registry.get_command_names(mode=CommandMode.CHAT, include_aliases=True):
