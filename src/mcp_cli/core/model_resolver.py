...
class ModelResolver:
    ...
    def resolve(
        ...
        if provider and model:
            ...
            logger.debug(f"Using provider with default model: {provider}/{default_model}")
            ...
            logger.debug(f"Using current provider with specified model: {current_provider}/{model}")
            ...
            logger.debug(f"Using active configuration: {active_provider}/{active_model}")
    ...
    def switch_to(self, provider: Optional[str] = None, model: Optional[str] = None) -> Tuple[str, str]:
        """
    ...
    def configure_provider(
        ...
        """
        self.model_manager.configure_provider(provider, api_key=api_key, api_base=api_base)
