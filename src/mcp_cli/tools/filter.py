...
class ToolFilter:
    ...
    def filter_tools(self, tools: List[Dict[str, Any]], provider: str = "openai") -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        ...
        for tool in tools:
            ...
            if not self.is_tool_enabled(tool_name):
                invalid_tools.append({
                    **tool,
                    "_disabled_reason": self.get_disabled_tools().get(tool_name, "unknown")
                })
                continue
            ...
            if provider == "openai":
                if self.auto_fix_enabled:
                    ...
                    is_valid, fixed_tool, error_msg = ToolSchemaValidator.validate_and_fix_tool(tool, provider)
                    ...
                    if is_valid:
                        ...
                        if fixed_tool != tool:
                            logger.info(f"Auto-fixed tool '{tool_name}' - removed unsupported properties")
                        
                        valid_tools.append(fixed_tool)
                        continue
                    else:
                        self._fix_stats["failed"] += 1
                        logger.warning(f"Tool '{tool_name}' failed validation even after auto-fix: {error_msg}")
                    ...
                    is_valid, error_msg = ToolSchemaValidator.validate_openai_schema(tool)
                    
                    if is_valid:
                        valid_tools.append(tool)
                    else:
                        logger.warning(f"Tool '{tool_name}' failed validation: {error_msg}")
                        self.disable_tool(tool_name, "validation")
                        invalid_tools.append({
                            **tool,
                            "_validation_error": error_msg,
        ...
        if self._fix_stats["attempted"] > 0:
            logger.info(f"Auto-fix results: {self._fix_stats['successful']}/{self._fix_stats['attempted']} tools fixed successfully")
    ...
    def _try_fix_tool(self, tool: Dict[str, Any], provider: str) -> Optional[Dict[str, Any]]:
        """
