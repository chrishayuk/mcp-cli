...
class ToolManager:
    ...
    def _determine_timeout(self, explicit_timeout: Optional[float]) -> float:
        ...
        for env_var in ["MCP_TOOL_TIMEOUT", "CHUK_TOOL_TIMEOUT", "MCP_CLI_INIT_TIMEOUT"]:
    ...
    def _detect_server_types(self):
        ...
        logger.info(f"Detected {len(self._http_servers)} HTTP, {len(self._sse_servers)} SSE, {len(self._stdio_servers)} STDIO servers")
    ...
    async def initialize(self, namespace: str = "stdio") -> bool:
        ...
        try:
            ...
            logger.error(f"Initialization timed out after {self.initialization_timeout}s")
            return False
            ...
            for ns, name in registry_items:
                try:
                    ...
                    tools.append(ToolInfo(
                        ...
                        supports_streaming=getattr(metadata, "supports_streaming", False) if metadata else False
                    ))
    ...
    async def get_tool_by_name(self, tool_name: str, namespace: str = None) -> Optional[ToolInfo]:
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any], timeout: Optional[float] = None) -> ToolCallResult:
        ...
        if not self.tool_filter.is_tool_enabled(tool_name):
            disabled_reason = self.tool_filter.get_disabled_tools().get(tool_name, "unknown")
            return ToolCallResult(tool_name, False, error=f"Tool disabled ({disabled_reason})")
        
        # CLEAN: Just look up the tool directly in the registry
        namespace, base_name = await self._find_tool_in_registry(tool_name)
        
        if not namespace:
            return ToolCallResult(tool_name, False, error=f"Tool '{tool_name}' not found in registry")
        
        logger.info(f"EXECUTION: Found tool '{tool_name}' -> namespace='{namespace}', base_name='{base_name}'")
        logger.info(f"EXECUTION: Arguments = {arguments}")
        logger.info(f"EXECUTION: Creating ToolCall with tool='{base_name}', namespace='{namespace}'")
        ...
        logger.info(f"EXECUTION: ToolCall created: tool='{call.tool}', namespace='{call.namespace}'")
        logger.info(f"EXECUTION: ToolCall arguments: {call.arguments}")
        
        try:
            import time
            logger.info(f"EXECUTION: Calling executor.execute() with call")
            start_time = time.time()
            results = await self._executor.execute([call])
            elapsed = time.time() - start_time
            logger.info(f"EXECUTION: Executor completed in {elapsed:.2f}s, returned {len(results) if results else 0} results")
            ...
            result = results[0]
            logger.info(f"EXECUTION: First result: success={not bool(result.error)}, error='{result.error}', result_type={type(result.result)}")
    ...
    async def _find_tool_in_registry(self, tool_name: str) -> Tuple[str, str]:
        ...
        try:
            ...
            logger.info(f"Looking for tool '{tool_name}' in {len(registry_items)} registry entries")
    ...
    async def process_tool_calls(
        ...
        for tc in tool_calls:
            ...
            tool_call_id = tc.get("id") or f"call_{llm_tool_name}_{uuid.uuid4().hex[:8]}"
            ...
            logger.debug(f"Tool call: LLM name='{llm_tool_name}' -> Original name='{original_tool_name}'")
            ...
            try:
                args_dict = json.loads(args_str) if isinstance(args_str, str) and args_str.strip() else args_str or {}
            ...
            call_mapping[id(call)] = {"id": tool_call_id, "name": llm_tool_name}  # Use LLM name for conversation
        ...
        for result in results:
            ...
            if conversation_history is not None:
                content = f"Error: {result.error}" if result.error else self.format_tool_response(result.result)
                conversation_history.append({
                    "role": "tool",
            ...
            if hasattr(self.stream_manager, 'get_server_info'):
                ...
                return [
                    ServerInfo(
                        ...
                        namespace=raw.get("name", "").split("_")[0] if "_" in raw.get("name", "") else raw.get("name", ""),
                    )
    ...
    async def get_adapted_tools_for_llm(self, provider: str = "openai") -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
        ...
        # Get raw tools first
        raw_tools, raw_name_mapping = await self._get_raw_adapted_tools_for_llm(provider)
        ...
        valid_tool_names = {self.tool_filter._extract_tool_name(tool) for tool in valid_tools}
        filtered_name_mapping = {
            k: v for k, v in raw_name_mapping.items() 
            if any(v.endswith(name.split('.')[-1]) or v == name for name in valid_tool_names)
        }
        ...
        if invalid_tools:
            logger.warning(f"Tool validation for {provider}: {len(valid_tools)} valid, {len(invalid_tools)} invalid")
            # Log specific errors for debugging
            for error in self.validation_results["validation_errors"]:
                logger.debug(f"Tool '{error['tool']}' validation error: {error['error']}")
        else:
            logger.info(f"Tool validation for {provider}: {len(valid_tools)} valid, 0 invalid")
        
        return valid_tools, filtered_name_mapping

    async def _get_raw_adapted_tools_for_llm(self, provider: str = "openai") -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
        ...
        for tool in unique_tools:
            ...
            name_mapping[tool_name] = tool.name  # Identity mapping - no conversion needed
    ...
    async def validate_single_tool(self, tool_name: str, provider: str = "openai") -> Tuple[bool, Optional[str]]:
        """
    ...
    def get_tool_validation_details(self, tool_name: str) -> Optional[Dict[str, Any]]:
        ...
        if tool_name in disabled_tools:
            ...
            return {
                ...
                "can_auto_fix": self.tool_filter.auto_fix_enabled and reason == "validation"
            }
        else:
            return {
                "tool_name": tool_name,
    ...
    async def list_prompts(self) -> List[Dict[str, Any]]:
        ...
        if self.stream_manager and hasattr(self.stream_manager, "list_prompts"):
            try:
                return await asyncio.wait_for(self.stream_manager.list_prompts(), timeout=10.0)
            except Exception:
                pass
        return []
    ...
    async def list_resources(self) -> List[Dict[str, Any]]:
        ...
        if self.stream_manager and hasattr(self.stream_manager, "list_resources"):
            try:
                return await asyncio.wait_for(self.stream_manager.list_resources(), timeout=10.0)
            except Exception:
                pass
        return []
