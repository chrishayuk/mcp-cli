...
class ToolSchemaValidator:
    ...
    def validate_openai_schema(tool_def: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not re.match(r'^[a-zA-Z0-9_-]+$', name):
                return False, f"Function name '{name}' contains invalid characters. Only a-z, A-Z, 0-9, _, - allowed"
    ...
    def _check_array_schemas(obj: Any, path: str = "") -> List[str]:
        ...
        if isinstance(obj, dict):
            ...
            if obj.get("type") == "array" and "items" not in obj:
                errors.append(f"Array schema at {path or 'root'} missing 'items' property")
                ...
                if schema_key in obj and isinstance(obj[schema_key], list):
                    for i, schema in enumerate(obj[schema_key]):
                        sub_errors = ToolSchemaValidator._check_array_schemas(
                            schema, f"{path}.{schema_key}[{i}]" if path else f"{schema_key}[{i}]"
                        )
            ...
            for key, value in obj.items():
                if key not in ["anyOf", "oneOf", "allOf"]:  # Already checked above
                    ...
                    sub_errors = ToolSchemaValidator._check_array_schemas(value, sub_path)
                    errors.extend(sub_errors)
    ...
    def fix_openai_compatibility(tool_def: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "function" in fixed and isinstance(fixed["function"], dict):
            ...
            unsupported_props = ["title", "examples", "deprecated", "version", "tags", "summary"]
            ...
            if removed_props:
                logger.debug(f"Removed unsupported properties {removed_props} from function '{function.get('name', 'unknown')}'")
            ...
            if "parameters" in function and isinstance(function["parameters"], dict):
                ...
                fixed["function"]["parameters"] = ToolSchemaValidator.fix_array_schemas(parameters)
                ...
                if param_removed:
                    logger.debug(f"Removed unsupported parameter properties {param_removed}")
        
        return fixed
    
    @staticmethod
    def validate_and_fix_tool(tool_def: Dict[str, Any], provider: str = "openai") -> Tuple[bool, Dict[str, Any], Optional[str]]:
        """
