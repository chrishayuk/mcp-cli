...
async def servers_action_async(
    ...
    if output_format == "json":
        ...
        # Show tip with icon
        output.tip(
            "💡 Use: /server <name> for details  |  /ping to test connectivity  |  /tools to see available tools"
        )
...
async def server_details_async(server_name: str) -> None:
    ...
    output.print(
        f"  [bold]Capabilities:[/bold]  {_format_capabilities(target_server.capabilities)}"
    )
    ...
    if target_server.tool_count > 0 and target_server.tool_count <= 10:
        try:
            tools = (
                await tm.get_tools_for_server(server_name)
