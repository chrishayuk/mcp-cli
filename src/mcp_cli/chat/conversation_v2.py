...
class ConversationProcessor:
    ...
    async def process_conversation(self):
        ...
        try:
            ...
            last_msg = self.context.conversation_history[-1] if self.context.conversation_history else {}
            ...
            if self._should_use_streaming():
                await self._handle_streaming_conversation(content)
            else:
                await self._handle_regular_conversation(content)
    ...
    async def _handle_streaming_conversation(self, user_message: str):
        ...
        try:
            ...
            with self._current_live_display:
                ...
                async for chunk in self.context.stream_with_tools(user_message):
                    ...
                    if self.ui_manager._interrupted:
            ...
            if collected_tool_calls:
                logger.debug(f"Processing {len(collected_tool_calls)} tool calls from stream")
    ...
    async def _handle_regular_conversation(self, user_message: str):
