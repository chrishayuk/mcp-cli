...
def main_callback(
    ...
    theme: str = typer.Option("default", "--theme", help="UI theme (default, dark, light, minimal, terminal)"),
) -> None:
    ...
    if provider and provider in provider_commands:
        ...
        output.tip(f"Use 'mcp-cli provider {provider}' or 'mcp-cli providers {provider}' instead")
...
def _setup_command_logging(quiet: bool, verbose: bool, log_level: str, theme: str = "default"):
...
def servers_command(
    ...
    if output_format.lower() not in valid_formats:
        output.error(f"Invalid format '{output_format}'. Must be one of: {', '.join(valid_formats)}")
...
def models_command(
    ...
    if provider_info.get('has_api_key'):
        output.print(f"[dim]API Key:[/dim] ********")
    else:
        output.print(f"[dim]API Key:[/dim] [red]Not configured[/red]")
